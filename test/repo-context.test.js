...
const { collectRepositoryContext, commitPendingChanges, isOnlyGitignoreChange, discoverRelevantFiles } = require('../server');
...
// Regression test for a second, distinct silent-failure mode reported after
// the .gitignore fix above: aider given a real, well-defined task ("move
// the images folder into public and fix the paths") made ZERO edits,
// because it was called with no --file args at all and had to rely purely
// on its symbol-based repo-map, which doesn't surface plain <img src="...">
// references inside HTML. discoverRelevantFiles grounds aider with the
// actual files instead of leaving it to guess.
test('discoverRelevantFiles finds files that textually reference a folder named in the task', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coding-agent-discover-'));
  execFileSync('git', ['init'], { cwd: tempDir, stdio: 'ignore' });
  fs.mkdirSync(path.join(tempDir, 'images'), { recursive: true });
  fs.mkdirSync(path.join(tempDir, 'public'), { recursive: true });
  fs.writeFileSync(path.join(tempDir, 'images', 'love.png'), 'fake-binary-content');
  fs.writeFileSync(
    path.join(tempDir, 'public', 'index.html'),
    '<html><body><img src="../images/love.png"></body></html>\n'
  );
  fs.writeFileSync(path.join(tempDir, 'README.md'), '# demo\nNo image mentions here.\n');
  execFileSync('git', ['add', '-A'], { cwd: tempDir, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tempDir, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.name', 'Test User'], { cwd: tempDir, stdio: 'ignore' });
  execFileSync('git', ['commit', '-m', 'initial'], { cwd: tempDir, stdio: 'ignore' });

  const found = await discoverRelevantFiles({
    workDir: tempDir,
    task: 'move images folder from root and place it inside the public folder and change the image paths to match it',
  });

  assert.ok(found.includes('public/index.html'), `expected public/index.html in ${JSON.stringify(found)}`);
  assert.ok(!found.includes('README.md'), `README.md should not match, got ${JSON.stringify(found)}`);
});

test('discoverRelevantFiles returns nothing when the task does not name a real top-level path', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coding-agent-discover-none-'));
  execFileSync('git', ['init'], { cwd: tempDir, stdio: 'ignore' });
  fs.writeFileSync(path.join(tempDir, 'README.md'), '# demo\n');
  execFileSync('git', ['add', '-A'], { cwd: tempDir, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tempDir, stdio: 'ignore' });
  ...
  const found = await discoverRelevantFiles({ workDir: tempDir, task: 'fix the login bug on the signup page' });
