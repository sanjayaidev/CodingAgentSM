...
// Aider is called with zero --file args, so by default it relies entirely
// on its own repo-map (built from code SYMBOLS via tree-sitter — functions,
// classes) to decide which files even exist. That map is a poor fit for
// tasks like "move the images folder and fix the paths": a plain
// `<img src="../images/x.png">` in an HTML file isn't a symbol, so the map
// may never surface that file to the model at all, and the model has no
// other way to discover it in a single non-interactive --message run.
//
// This does a cheap, best-effort grounding pass: pull plausible keywords
// out of the task text, match them against real top-level paths in the
// repo, then `git grep` the repo for textual references to those paths.
// Anything found is passed to aider as explicit positional file args —
// exactly what happens when a human runs `aider file1 file2` or uses
// `/add` — so the model has the actual file content in its context
// instead of having to intuit relevance from a symbol map.
async function discoverRelevantFiles({ workDir, task }) {
  let topLevel = [];
  try {
    topLevel = fs.readdirSync(workDir, { withFileTypes: true })
  ...
  const matchedNames = topLevel.filter((name) => {
    const base = normalize(name);
    const singular = base.endsWith('s') ? base.slice(0, -1) : base;
    return taskLower.includes(base) || taskLower.includes(singular);
  });
  ...
  for (const name of matchedNames) {
    try {
      // -I skips binary files (no point handing aider a PNG's bytes);
      // git grep exits 1 with empty stdout when there are no matches, which
      // is fine — just nothing to add for that name.
      const { stdout } = await run('git', ['grep', '-l', '-i', '-I', '--', name], { cwd: workDir });
      stdout.trim().split('\n').filter(Boolean).forEach((f) => relevantFiles.add(f));
    } catch (_) {
      // no matches, or name isn't valid grep pattern (e.g. contains a
      // space) — not fatal, just skip it
    }
  }
  return [...relevantFiles].slice(0, 25); // cap so this can't blow up the aider invocation
}
...
async function invokeAider({ workDir, logsDir, chosenModel, taskText, aiderEnv, relevantFiles = [] }) {
  const aiderArgs = [
    // Explicit files first — see discoverRelevantFiles above. Plain relative
    // paths never look like flags to aider's argparse, so order relative to
    // the options below doesn't matter, but putting them first mirrors how
    // a human would type `aider file1 file2 --message ...`.
    ...relevantFiles,
    '--yes-always',
    '--no-check-update',
...
async function runAgentTask({
  ...
  try {
    ...
    const relevantFiles = await discoverRelevantFiles({ workDir, task }).catch(() => []);
    if (relevantFiles.length > 0) {
      step(`Adding ${relevantFiles.length} likely-relevant file(s) to aider's context: ${relevantFiles.join(', ')}`);
    }
    let aiderResult = await invokeAider({ workDir, logsDir, chosenModel, taskText: fullTask, aiderEnv, relevantFiles });
